...
@Component({
  selector: 'app-phieu-dang-ky',
  templateUrl: './phieu-dang-ky.component.html',
  styleUrls: ['./phieu-dang-ky.component.scss']
})
export class PhieuDangKyComponent extends BaseClass implements OnInit {
  @Input() data: any = [];
  displayedColumns: string[] = ['stt', 'formCode', 'content', 'subscribers', 'regDate', 'regDepartment', 'detail', 'action'];
  ...
  constructor(
    ...
    this.donViService.getDonVi(this.searchDonVi).pipe(this.unsubsribeOnDestroy).subscribe(
      (res: any) => {
  ...
  search() {
    ...
    this._service.getListPhieuDangKy(param).pipe(this.unsubsribeOnDestroy).subscribe(
      (rs: any) => {
        if (rs.success) {
  ...
  addFile(id, item, vitri) {
    this._service.getChiTietPhieuVPP(id).pipe(this.unsubsribeOnDestroy).subscribe(
      (res: any) => {
        if (res.success) {
