...
import { HINH_THUC_TT, MUC_DO_UU_TIEN, TRANG_THAI_HO_SO } from '@app/shared/constants/hosothanhtoan.const';
import * as util from '@app/shared/constants/util';
@Component({
    selector: 'app-ho-so-chua-phe-duyet',
    templateUrl: './ho-so-chua-phe-duyet.component.html',
    styleUrls: ['./ho-so-chua-phe-duyet.component.scss']
})
export class HoSoChuaPheDuyetComponent extends BaseClass implements OnInit {
    ...
    view(id: string) {
        this.router.navigate([`/ho-so-thanh-toan/chitiethoso/${id}`, { action: 'APPROVE' }]);
    }
}
